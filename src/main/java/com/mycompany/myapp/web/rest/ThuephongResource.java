...
public class ThuephongResource {
    ...
    private final ThuephongService thuephongService;

    private final ThuephongRepository thuephongRepository;

    public ThuephongResource(ThuephongService thuephongService, ThuephongRepository thuephongRepository) {
        this.thuephongService = thuephongService;
    ...
    public ResponseEntity<Thuephong> createThuephong(@RequestBody Thuephong thuephong) throws URISyntaxException {
        ...
        Thuephong result = thuephongService.save(thuephong);
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/thuephongs/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<Thuephong> partialUpdateThuephong(
        ...
        Optional<Thuephong> result = thuephongService.partialUpdate(thuephong);
    ...
    public ResponseEntity<List<Thuephong>> getAllThuephongs(Pageable pageable) {
        ...
        Page<Thuephong> page = thuephongService.findAll(pageable);
    ...
    public ResponseEntity<Thuephong> getThuephong(@PathVariable Long id) {
        ...
        Optional<Thuephong> thuephong = thuephongService.findOne(id);
    ...
    public ResponseEntity<List<Thuephong>> searchThuephongs(@RequestParam String query, Pageable pageable) {
        ...
        Page<Thuephong> page = thuephongService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
